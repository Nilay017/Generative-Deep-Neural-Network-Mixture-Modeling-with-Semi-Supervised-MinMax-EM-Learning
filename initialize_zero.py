...
def main():
    ...
    parser.add_argument("-r", "--run_name", dest="run_name", default='0.2_end2end_clusgan_3_123_pretrain_moreGAN', help="Name of training run")
    ...
    datafile_path = "./Output_data_processing/kmeans_clustering_data_3_0.1.pkl"
    ...
    supervision_level = given_supervision_level
    ...
    sep_und = '_'
    run_name_comps = ['%fsupervision--'%supervision_level,'%iepoch'%n_epochs, 'z%s'%str(latent_dim), mtype, 'bs%i'%batch_size, run_name]
    ...
    for epoch in range(n_epochs):
        ...
        for i, (imgs, itruth_label) in enumerate(dataloader):
            ...
            sampled_z = sample_z(shape=imgs.shape[0], latent_dim=latent_dim, n_c=3, fix_class=-1, cluster_centers=cluster_centers, req_grad=False)
    ...
    for epoch in range(n_epochs):
        ...
        for i, (imgs, itruth_label) in enumerate(dataloader_correct):
            ...
            # Sample random latent variables
            sampled_z_new = sample_z(shape=imgs.shape[0], latent_dim=latent_dim, n_c=3, fix_class=-1, cluster_centers=cluster_centers, req_grad=False)
    ...
    for epoch in range(n_epochs):
        ...
        for i, (imgs, itruth_label) in enumerate(dataloader_only_unlabelled):
            ...
            # logProb2 = logProb.detach()
            # Posterior = F.softmax(logProb2, -1)
            # Posterior[fixed_indices] = torch.eye(cluster_centers.shape[0]).astype('float32')[fixed_gt_idx]
            # MLE_loss = -torch.sum(logProb*Posterior)

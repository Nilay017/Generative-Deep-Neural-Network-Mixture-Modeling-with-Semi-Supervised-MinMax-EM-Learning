...
def sample_z(shape=64, latent_dim=3, n_c=3, fix_class=-1, sigma=1.0, cluster_centers=torch.tensor([[1., 0., 0.], [0., 1., 0.], [0., 0., 1.]]), req_grad=False):
    ...
    zn = Variable(Tensor(0.75*np.random.normal(0, sigma, (shape, latent_dim))), requires_grad=req_grad)
    ...
    return zn + cluster_centers[zc_idx.to(torch.long)].cuda()

...
parser.add_argument("-r", "--run_name", dest="run_name", default='pretrain_clusgan_7_all', help="Name of training run")
...
def run_main(seed):
    # python3 em_clusgan_copy.py -n 300 -g 0 -sup 0.0 -seed 1.0
    global args
    ...
    datafile_path = "./Output_data_processing/kmeans_clustering_data_7_" + str(given_supervision_level) + "_seed_" + str(int(seed)) + ".pkl"
    ...
    dataMNIST = CustomMNISTdigit(finaldata, finaltarget, transform)
    ...
    model_loadpath = '/home/nilay/clusterGAN-master_2/runs/mnist/pretraining_supervision_'
    ...
    # model_loadpath += str(0.4) + '00000'
    # assert(supervision_level == 0.0)
    model_loadpath += "__epoch_300____van___11-2___seed_" + str(int(seed)) + "__pretrain_clusgan_7_all/models/"
    # model_loadpath += '__epoch_300____van___11-18-01_pretrain_clusgan_3_123/models/'
    ...
    log_final_results = open("train_clusgan_MNIST_7_digits_semi_sup_" + str(supervision_level) + "_seed_" + str(seed), "a+")
    ...
    if supervision_level > 0.0:
        ...
        fixed_data_0 = finaldata[fixed_indices]
        ...
        fixed_data = torch.zeros([fixed_data_0.shape[0], 1, fixed_data_0.shape[1], fixed_data_0.shape[2]]).type(Tensor)
        for i in range(fixed_data_0.shape[0]):
            fixed_data[i, 0] = transform(transform2(fixed_data_0[i]))
    ...
    t_imgs, t_label = finaldata.clone(), finallabels.clone()
    test_data = torch.zeros([t_imgs.shape[0], 1, t_imgs.shape[1], t_imgs.shape[2]]).type(Tensor)
    for i in range(t_imgs.shape[0]):
        test_data[i, 0] = transform(transform2(t_imgs[i]))
...
if __name__ == "__main__":
    ...
    # seeds = [0., 1.]
    # supervision_levels = [0.0, 0.1]
    # for given_supervision_level in supervision_levels:
    #     final_dict[given_supervision_level] = {}
    log_final_results_metrics = open("metrics_train_clusgan_MNIST_7_digits_semi_sup_" + str(args.supervision_level), "a+")
    ...
    pkl.dump(final_dict, open( './new_final_dict_7_' + str(args.supervision_level) + '_seed_' + str(seed) + '.pkl', "wb"), protocol=pkl.HIGHEST_PROTOCOL)
    print("Dumping into  ./new_final_dict_7_" + str(args.supervision_level) + '_seed_' + str(seed) + ".pkl")
